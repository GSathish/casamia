...
/*----------------------------------------------------------------------------*/
coap_observee_t *
coap_add_obs_subject(uip_ipaddr_t * addr, uint16_t port,
                     ...
                     void *data)
{
  ...
  coap_remove_obs_subject_by_url(addr, port, url);
...
/*----------------------------------------------------------------------------*/
void
coap_remove_obs_subject(coap_observee_t * o)
{
...
/*----------------------------------------------------------------------------*/
int
coap_remove_obs_subject_by_token(uip_ipaddr_t * addr, uint16_t port,
...
/*----------------------------------------------------------------------------*/
int
coap_remove_obs_subject_by_url(uip_ipaddr_t * addr, uint16_t port,
                               const char *url)
{
  int removed = 0;
  coap_observee_t *obs = NULL;
                         ...
                         coap_packet_t * notification)
{
  ...
  PRINTF("coap_notification_callback()\n");
  pkt = (coap_packet_t *) notification;
  token_len = coap_get_header_token(pkt, &token);
  ...
  PRINTF("Getting obs_subject info\n");
  ...
  if(obs->notification_callback != NULL) {
    ...
    // TODO: the following technique for discarding duplicates is too trivial
...
/*----------------------------------------------------------------------------*/
uint8_t
coap_get_token(uint8_t ** token_ptr)
{
                              ...
                              void *data)
{
  ...
  token_len = coap_get_token(&token);
  coap_set_header_token(request, token, token_len);
  t = coap_new_transaction(request->mid, addr, port);
  if(t) {
    obs = coap_add_obs_subject(addr, port, (uint8_t *) token, token_len, uri,
